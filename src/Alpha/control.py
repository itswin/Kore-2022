...
def direct_attack(agent: Player, max_distance: int = 10, max_time_to_wait: int = 5):
    ...
    for t in targets:
        ...
        shipyards.sort(key=lambda x: x.distance_from(t) + x.calc_time_for_ships_for_action(min_ships_to_send))
        for sy in shipyards:
            ...
            for target_time, target_point in enumerate(t.route, 1):
                ...
                # Check the direct attack point, then check adjacent points
...
def direct_attack_expansion(agent: Player, max_distance: int = 10, max_time_to_wait: int = 10):
    board = agent.board
    ...
    for x in agent.opponents:
        for sy in x.all_shipyards:
            for fleet in sy.incoming_allied_fleets:
                if fleet.route.last_action() == Convert:
                    targets.append(fleet)
    ...
    for t in targets:
        min_ships_to_send = int(t.ship_count + 1 - 50)
        max_ships_to_send = int(t.ship_count)
        attacked = False
        best_candidate_sy = None
        best_candidate_time = max_time_to_wait
        best_target_point = None
        ...
        for sy in shipyards:
            ...
            num_ships_to_launch = min(sy.available_ship_count, max_ships_to_send)
            ...
            for target_time, target_point in enumerate(t.route, 1):
                ...
                for target_point in target_points:
                    ...
                    route = random.choice(routes)
                    logger.info(
                        f"Direct attack expansion {sy.point}->{target_point}, distance={target_time}"
                    )
                    sy.action = Launch(num_ships_to_launch, route)
                    attacked = True
                if attacked:
        ...
        if not attacked:
            if best_candidate_sy is not None:
                _spawn(agent, best_candidate_sy)
                logger.info(f"Saving for direct attack expansion {t.point}, {best_candidate_sy.point}->{best_target_point}, distance={best_candidate_time}")
